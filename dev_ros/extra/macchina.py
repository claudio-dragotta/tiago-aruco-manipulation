...
class MacchinaStati(Node):
    ...
    def inverse_kinematics_jacobian(self, dh_params, joint_angles, target_position, target_velocity, joint_limits, K=1.0, max_iterations=500, tol=1e-2):
        ...
        for _ in range(max_iterations):
            ...
            # Creiamo un vettore di errore completo 6D (posizione e orientamento)
            error_full = np.zeros(6)
            error_full[:3] = error  # Solo errore di posizione per ora
            ...
            # Creiamo un vettore velocità completo 6D
            velocity_full = np.zeros(6)
            velocity_full[:3] = target_velocity
            
            # Calcolo con vettori 6D completi
            dq = np.dot(J_pinv, (velocity_full + K * error_full))
            q = q + dq * 0.5
