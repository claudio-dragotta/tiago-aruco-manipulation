...
class MacchinaStati(Node):
    def __init__(self):
        ...
        self.robot = ERobot.URDF('/home/claudio/progetto_ros2/dev_ros/tiago_robot.urdf')
        ...
        self.get_logger().info('🧠 Nodo macchina_a_stati doppio_task avviato.')
    ...
    def calcola_cinematica_inversa(self, pose_stamped):
        ...
        T = SE3(pose.position.x, pose.position.y, pose.position.z) * SE3.RPY([0, 0, 0], order='xyz', unit='rad')
        sol = self.robot.ikine_LM(T)
        if not sol.success:
            self.get_logger().warn("⚠️ IK fallita. Uso configurazione di fallback.")
        ...
        match self.state:
            ...
            case 3:
                if 3 in self.posizioni:
                    self.get_logger().info('🟡 Stato 3: porto oggetto 1 a destinazione (marker 3)')
                    config = self.calcola_cinematica_inversa(self.posizioni[4])
                    self.invia_trajectory(config)
                    self.state = 4

            case 4:
                self.get_logger().info('🤖 Stato 4: apro gripper (rilascio oggetto 1)')
            ...
            case 7:
                if 4 in self.posizioni:
                    self.get_logger().info('🟡 Stato 7: porto oggetto 2 a destinazione (marker 4)')
                    config = self.calcola_cinematica_inversa(self.posizioni[3])
                    self.invia_trajectory(config)
                    self.state = 8

            case 8:
                self.get_logger().info('🤖 Stato 8: apro gripper (rilascio oggetto 2)')
                self.controlla_gripper(chiudi=False)
                self.state = 9

            case 9:
                self.get_logger().info('✅ Sequenza COMPLETATA con successo')
                self.timer.cancel()
