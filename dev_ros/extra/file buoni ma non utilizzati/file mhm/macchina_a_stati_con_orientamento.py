...
class MacchinaStati(Node):
    def __init__(self):
        super().__init__('macchina_a_stati_orientamento_debug')
        self.state = 0
        self.posizioni = {}
        ...
        self.get_logger().info('🧠 Nodo macchina con orientamento avviato.')
    ...
    def calcola_cinematica_inversa(self, pose_stamped):
        ...
        if norm > 0:
            ...
            self.get_logger().warn("❗ Quaternione nullo, uso [0, 0, 0, 1]")
        ...
        if not success:
            self.get_logger().warn("⚠️ IK con orientamento fallita. Uso fallback.")
